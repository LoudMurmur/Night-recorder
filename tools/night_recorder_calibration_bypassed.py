...
"""
calibration is bypassed, detection level is fixed at 150
"""
...
def recalibrate_for_ambiant_noise(stream):
    ...
    for step in range(N_CHUNK_FOR_CALIBRATION):
        ...
        if DISPLAY:
            ...
            callim = cv2.imread(os.path.join(base, "ims/call003.bmp"))
    ...
    new_threshold = int(getAverageValue(measurments)*1.3) #setting threshold to 30% higher than the ambiant level
...
while True:
    ...
    if silence_start_time is not None:
        ...
        print "Microphone is sensing silence since : {}s".format(current_silence_duration_seconds)
    ...
    if recording_start_time is not None:
        ...
        print "Microphone is sensing sound since : {}s".format(current_recording_duration_seconds)
    ...
    if DISPLAY:
        thisfilepath = os.path.realpath(__file__)
        base = os.path.split(thisfilepath)[0]
        callim = cv2.imread(os.path.join(base, "ims/call004.bmp"))
        clean_voice_volbar = makeVolumeBar(average_voice_level_no_ambiant_amped, computed_detection_level)
        copyImageIntoOtherImage(callim, clean_voice_volbar, Point(0,200))
        cv2.imshow("NightRec", callim)
        cv2.waitKey(1) # this block the execution for 1ms :(
    ...
    if currently_recording:
        ...
        if silent_blocks > N_SILENT_CHUNK_TO_STOP:
            ...
            if record_lasted_seconds > FALSE_DETECTION_MIN_TIME_SECONDS:
                ...
                recording_name = str(recording_number).zfill(10)
