...
N_CHUNK_FOR_CALIBRATION = 200
N_SILENT_PRE_CHUNK_TO_KEEP = 50
N_SILENT_CHUNK_TO_STOP = 50
FALSE_DETECTION_MIN_TIME_SECONDS = 1.7
...
def makeProgressBar(currentStep, totalStep, height, width):
    
    stepWidth = width/totalStep
    color = (255,0,0)
    return createEmptyBgrColoredImage(currentStep*stepWidth, height, color)
...
def recalibrate_for_ambiant_noise(stream):
    ...
    measurments = []
    for step in range(N_CHUNK_FOR_CALIBRATION):
        n, sound_data = read_available(stream)
        instant_voice_level = computeVoiceLevelViaFFT(n, sound_data, MAX_y)
        measurments.append(instant_voice_level)
        
        if DISPLAY:
            thisfilepath = os.path.realpath(__file__)
            base = os.path.split(thisfilepath)[0]
            callim = cv2.imread(os.path.join(base, "ims/calibration_im.bmp"))
            volbarim = makeVolumeBar(instant_voice_level, getAverageValue(measurments))
            copyImageIntoOtherImage(callim, volbarim, Point(0, 199))
            copyImageIntoOtherImage(callim, makeProgressBar(step, N_CHUNK_FOR_CALIBRATION, 5, 1400), Point(0,170))
            cv2.imshow("NightRec", callim)
            cv2.waitKey(1) # this block the execution for 1ms :(

    current_ambiant_level = getAverageValue(measurments)
    new_threshold = int(getAverageValue(measurments)*1.3) #setting threshold to 40% higher than the ambiant level
...
recording_number = 0
listening_im = cv2.imread("ims/listening_im.bmp")
recording_im = cv2.imread("ims/recording_im.bmp")

if DISPLAY:
    cv2.namedWindow("NightRec")

ambiant_volume_level, computed_detection_level = recalibrate_for_ambiant_noise(stream)
print "new threshold is {}".format(computed_detection_level)
...
while True:
    ...
    if silence_start_time is not None:
        ...
        #print "Mic is sensing silence since : {}s".format(current_silence_duration_seconds)
    ...
    if recording_start_time is not None:
        ...
        #print "Mic is sensing sound since : {}s".format(current_recording_duration_seconds)
        if current_recording_duration_seconds >= FIVE_MINUTES_SECONDS:
            ambiant_volume_level, computed_detection_level = recalibrate_for_ambiant_noise(stream)

    n, sound_data = read_available(stream)
    voice_inst_sound_level = computeVoiceLevelViaFFT(n, sound_data, MAX_y)
    ...
    average_voice_level_no_ambiant_amped = average_voice_level_no_ambiant*10

    if DISPLAY:
        if not currently_recording:
            display_im = listening_im
        else:
            display_im = recording_im
        clean_voice_volbar = makeVolumeBar(average_voice_level_no_ambiant_amped, computed_detection_level)
        copyImageIntoOtherImage(display_im, clean_voice_volbar, Point(0,200))
        cv2.imshow("NightRec", display_im)
        cv2.waitKey(1) # this block the execution for 1ms :(
    ...
    voice_detected = average_voice_level_no_ambiant_amped > computed_detection_level
    if voice_detected:
        if not currently_recording:
    ...
    if currently_recording:
        ...
        if silent_blocks > N_SILENT_CHUNK_TO_STOP:
            #IF recording time < FALSE_DETECTION_MIN_TIME_SECONDS seconds : discard
            record_lasted_seconds = time.time() - recording_start_time
            if record_lasted_seconds > FALSE_DETECTION_MIN_TIME_SECONDS:
                ...
                record_to_file(p.get_sample_size(FORMAT), to_save, 'recordings/recording{}.wav'.format(recording_name))
            else:
